...
?>
<title>Vendor Enquiry Reports</title>
<?php
      ...
      <center> <img width=85 height=85 style="" src="<?php echo base_url();?>/assets/images/slogo.png"></center> </td>
          ...
          <!--<p style="margin-left:20px;"><b>Report Run Date :&nbsp;</b><?php echo date('Y-m-d');?></p>-->
           <p style="margin-left:20px;"><b>Report Run Date :&nbsp;</b><?php echo date("d-m-Y", strtotime(date('Y-m-d')));?></p>
			<p style="margin-left:20px;"><b>Report Type  :&nbsp;</b> Vendor Enquiry Report</p>
...
<table width='90%' align='center' border=1 cellpadding=3>
  <thead>
    <tr>
      ...
      <th style='background-color:#d8d3d3;' >City</th>
      <th style='background-color:#d8d3d3;' >Address</th>
      <th style='background-color:#d8d3d3;' >Category</th>
  ...
  <tbody>
				...
				<?php
				 $i=0;
					...
					{
						$i++;
						$tbl_category_master = $this->db->get_where('tbl_category_master',array('category_id' => $value['category_id']))->result_array();
				?>
                <tr>
                    <td><?php echo $i;?></td>
...
<a href="<?php echo site_url('index.php/Home/excel_vendor_enquiry/'.$from_date.'/'.$to_date.''); ?>"> <input type="button" class="button" style='width: 200px;' value="Export To Excel"> </a>
</div>
...
{
          ?>
			<h3 align=center>Vendor Enquiry Reports</h3>
